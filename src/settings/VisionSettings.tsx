...
export function VisionSettings() {
  ...
  const selectProvider = (next: LLMProviderType) => { setProvider(next); setModel(""); setModels([]); setOpenRouterModels([]); };
  const selectModel = (next: string) => { setModel(next); };
  const available = selectedProvider === "openrouter" ? openRouterModels.map((m) => ({ id: m.id, name: m.name })) : models;
  const modelOptions = selectedModel && !available.some((item) => item.id === selectedModel)
    ? [{ id: selectedModel, name: selectedModel }, ...available]
    : available;

  return <div className="space-y-6">
    <div className="flex items-center gap-3 rounded-xl border border-primary/30 bg-primary/5 px-5 py-3.5">
      <Camera className="h-4 w-4 text-primary" />
      <div><p className="text-sm font-medium">Vision: {selectedProvider} {selectedModel ? `/ ${selectedModel}` : ""}</p><p className="text-xs text-muted-foreground">Choose the model used for screenshot analysis.</p></div>
    </div>
